...
Recom <- read.csv("~/associationrules.csv")
Recom <- lapply(Recom, function(x) as.factor(x))
Recom<-read.transactions("~/associationrules.csv")
Recom<-Recom[,-1]
...
#As is common in association rule mining, given a set of itemsets, the algorithm attempts to find subsets which are common to at least a minimum number C of the itemsets.
...
#Apriori uses breadth-first search and a tree structure to count candidate item sets efficiently. It generates candidate item sets of length k from item sets of length k-1.
...
rules <- apriori(Recom, parameter = list(minlen=2, supp=0.004, conf=0.9))
